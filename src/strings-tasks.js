...
function removeFirstOccurrences(str, value) {
  ...
  return str.replace(str.slice(indexOfValue, indexOfValue + value.length), '');
}
...
function formatTime(minutes, seconds) {
  const minutesToTime = minutes.toString().padStart(2, '0');
  const secondsToTime = seconds.toString().padStart(2, '0');
  return minutesToTime.concat(':', secondsToTime);
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (strCharCode <= 90 && strCharCode >= 65) {
      rotStr += String.fromCharCode(((strCharCode - 65 + 13) % 26) + 65);
    } else if (strCharCode <= 122 && strCharCode >= 97) {
      rotStr += String.fromCharCode(((strCharCode - 97 + 13) % 26) + 97);
    } else {
      rotStr += str.charAt(i);
    }
  }
